...
function evaluateSeven(cards) {
  if (cards.length < 5) return { category: 0, tiebreakers: cards.map((card) => card.value).sort((a, b) => b - a), name: "高牌" };
...
function evaluateFive(cards) {
  ...
  if (groups[0].count === 2 && groups[1]?.count === 2) return rank(2, [groups[0].value, groups[1].value, kicker(values, [groups[0].value, groups[1].value])], "两对");
